...
if grep -q 'MyAppVersion "0.3.1-beta"' "$WIN_ISS" 2>/dev/null; then
    record_pass "Windows installer script version: 0.3.1-beta (projectx.iss)"
else
    record_fail "Windows installer script version not 0.3.1-beta"
fi
